...
void BASE_X::SetMode(uint8_t pos, uint8_t mode) {
    pos = CheckPos(pos);
    bool res = M5.I2C.writeByte(BASE_X_ADDR, BASE_X_CONFIG_ADDR + (0x10 * pos), mode);
}
 ...
 * @return:
 */
bool BASE_X::SetMotorSpeed(uint8_t pos, int8_t duty) {
    uint8_t addr;
    pos  = CheckPos(pos);
    addr = BASE_X_PWM_DUTY_ADDR + pos;

    return M5.I2C.writeByte(BASE_X_ADDR, addr, duty);
}
